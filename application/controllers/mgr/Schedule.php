...
class Schedule extends Base_Controller
{
    ...
    private $local_th_title         = ['#','司機', '訂單狀態', '預約時間','發送城市','發送地址','收件城市', '收件地址', '動作'];
   ...
   // private $type                   = ['BRIDAL-GOWNS', 'EVENING-GOWNS'];
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . 'mgr/schedule/';
    }
    ...
    public function index()
    {
        ...
        $this->data = array_merge($this->data, array(
            ...
            'custom_del_url'          => base_url() . 'mgr/schedule/local_del',
            'title'                   => '列表',
            'action'                  => $this->action,
            ...
            'tool_btns'               => [
                ...
                ['add schedule', base_url() . 'mgr/schedule/add', 'btn-primary'],
            ],
        ));
    ...
    public function data()
    {
        ...
        $data = $this->schedule_model->get_local_list($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/schedule_local_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            var_dump($data);
            if ($this->schedule_model->add($data) !== FALSE) {


                $this->js_output_and_redirect("新增成功", base_url() . "mgr/schedule/index/");
            } else {
            ...
            $this->data['sub_active'] =  'schedule';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/schedule/index/";

            $this->data['action'] = base_url() . "mgr/schedule/add/";
    ...
    public function edit($id)
    {
        ...
        require('./vendors/autoload.php');
        //require('C:/xampp/htdocs/new_wedding/vendors/autoload.php');
        $schedule = $this->schedule_model->get_data($id);
        //var_dump($schedule);
        //exit;
        if ($_POST) {
            $data = $this->process_post_data($this->param);

            if ($this->schedule_model->edit($id, $data)) {

                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/schedule/");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            //$this->data['pics'] = $this->schedule_model->get_banner($id);
            $this->data['type'] = 'edit';
            $this->data['param'] = $this->set_data_to_param($this->param, $schedule);
            $this->data['title'] = '編輯照片';
            $this->data['sub_active'] =  'schedule';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/schedule/";
            $this->data['select']['type'] = array(['id' => 'EVENING-GOWNS', 'title' => 'EVENING-GOWNS'], ['id' => 'BRIDAL-GOWNS', 'title' => 'BRIDAL-GOWNS']);
            
            $this->data['action'] = base_url() . "mgr/schedule/edit/" . $id;
            $this->data['submit_txt'] = "確認編輯";
            //$this->data['select']['type'] = array(['id' => 'all_schedule', 'title' => 'all_schedule'], ['id' => 'ready_to_wear', 'title' => 'ready_to_wear'],['id' => 'customize', 'title' => 'customize'],['id' => 'schedule', 'title' => 'schedule']);
            // TODO: 新欄位還沒串
            //$this->data['select']['category'] = $this->schedule_model->get_hobby_schedule_classify();
    ...
    public function local_del($id)
    {
        ...
        if ($this->schedule_model->edit($id, array('is_delete' => 1))) {
            //$this->output(TRUE, 'success');
            $this->js_output_and_redirect("刪除成功", base_url() . "mgr/schedule/");
        } else {
    ...
    // ----------------------------------------------------------------文章列表 start
    public function post_list($schedule_id)
    {
        $schedule = $this->schedule_model->get_data($schedule_id);
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'schedule_MGR_LOCAL',
            'parent'                  => ($schedule['type'] == 'local') ? '在地獵場列表' : '同好獵場',
            'parent_link'             => ($schedule['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
            'custom_data_url'          => base_url() . 'mgr/schedule/post_data/' . $schedule_id,
            'title'                   => '【' . $schedule['name'] . '】的文章列表',
            'action'                  => $this->action,
    ...
    public function post_data($schedule_id)
    {
        $page        = ($this->input->post("page"))            ? $this->input->post("page")        : 1;
        $search      = ($this->input->post("search"))        ? $this->input->post("search")        : '';
        $order       = ($this->input->post("order"))        ? $this->input->post("order")        : 0;
        ...
        $syntax    = "P.is_delete=0 AND post_at='schedule' AND relation_id='$schedule_id'";
        if ($status != 'ALL')
        ...
        $lists = $this->schedule_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
    ...
    public function hobby_add()
    {
        if ($_POST) {
            ...
            $this->load->view("mgr/schedule_form", $this->data);
        }
    }
