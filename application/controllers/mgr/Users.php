...
class users extends Base_Controller
{
    ...
    private $local_th_title         = ['#', '姓名', '電話','住址','生日','密碼', '信箱', '動作'];
   ...
   // private $type                   = ['BRIDAL-GOWNS', 'EVENING-GOWNS'];
    ...
    public function __construct()
    {
        ...
        $this->action = base_url() . 'mgr/users/';
    }
    ...
    public function index()
    {
        ...
        $this->data = array_merge($this->data, array(
            ...
            'custom_del_url'          => base_url() . 'mgr/users/local_del',
            'title'                   => '圖片列表',
            'action'                  => $this->action,
            ...
            'tool_btns'               => [
                ...
                ['add users', base_url() . 'mgr/users/add', 'btn-primary'],
            ],
        ));
    ...
    public function data()
    {
        ...
        $data = $this->users_model->get_local_list($syntax, $order_by, $page, $this->page_count);
        $html = "";
        foreach ($data['list'] as $item) {
            $html .= $this->load->view("mgr/items/users_local_item", array(
                "item" =>    $item,
            ), TRUE);
        }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            var_dump($data);
            if ($this->users_model->add($data) !== FALSE) {


                $this->js_output_and_redirect("新增成功", base_url() . "mgr/users/index/");
            } else {
            ...
            $this->data['sub_active'] =  'users';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/users/index/";

            $this->data['action'] = base_url() . "mgr/users/add/";
    ...
    public function edit($id)
    {
        ...
        require('./vendors/autoload.php');
        //require('C:/xampp/htdocs/new_wedding/vendors/autoload.php');
        $users = $this->users_model->get_data($id);
        //var_dump($users);
        //exit;
        if ($_POST) {
            $data = $this->process_post_data($this->param);

            if ($this->users_model->edit($id, $data)) {

                $this->js_output_and_redirect("編輯成功", base_url() . "mgr/users/");
            } else {
                $this->js_output_and_back("發生錯誤");
            }
        } else {
            //$this->data['pics'] = $this->users_model->get_banner($id);
            $this->data['type'] = 'edit';
            $this->data['param'] = $this->set_data_to_param($this->param, $users);
            $this->data['title'] = '編輯照片';
            $this->data['sub_active'] =  'users';

            $this->data['parent'] = '圖片列表';
            $this->data['parent_link'] = base_url() . "mgr/users/";
            $this->data['select']['type'] = array(['id' => 'EVENING-GOWNS', 'title' => 'EVENING-GOWNS'], ['id' => 'BRIDAL-GOWNS', 'title' => 'BRIDAL-GOWNS']);
            
            $this->data['action'] = base_url() . "mgr/users/edit/" . $id;
            $this->data['submit_txt'] = "確認編輯";
            //$this->data['select']['type'] = array(['id' => 'all_users', 'title' => 'all_users'], ['id' => 'ready_to_wear', 'title' => 'ready_to_wear'],['id' => 'customize', 'title' => 'customize'],['id' => 'users', 'title' => 'users']);
            // TODO: 新欄位還沒串
            //$this->data['select']['category'] = $this->users_model->get_hobby_users_classify();
    ...
    public function local_del($id)
    {
        ...
        if ($this->users_model->edit($id, array('is_delete' => 1))) {
            //$this->output(TRUE, 'success');
            $this->js_output_and_redirect("刪除成功", base_url() . "mgr/users/");
        } else {
    ...
    // ----------------------------------------------------------------文章列表 start
    public function post_list($users_id)
    {
        $users = $this->users_model->get_data($users_id);
        $this->data = array_merge($this->data, array(
            'sub_active'              => 'users_MGR_LOCAL',
            'parent'                  => ($users['type'] == 'local') ? '在地獵場列表' : '同好獵場',
            'parent_link'             => ($users['type'] == 'local') ? base_url() . 'mgr/huntground/local' : base_url() . 'mgr/huntground/hobby',
            'custom_data_url'          => base_url() . 'mgr/users/post_data/' . $users_id,
            'title'                   => '【' . $users['name'] . '】的文章列表',
            'action'                  => $this->action,
    ...
    public function post_data($users_id)
    {
        $page        = ($this->input->post("page"))            ? $this->input->post("page")        : 1;
        $search      = ($this->input->post("search"))        ? $this->input->post("search")        : '';
        $order       = ($this->input->post("order"))        ? $this->input->post("order")        : 0;
        ...
        $syntax    = "P.is_delete=0 AND post_at='users' AND relation_id='$users_id'";
        if ($status != 'ALL')
        ...
        $lists = $this->users_model->get_post_list($syntax, $order_by, intval($page), $page_count = 20);
    ...
    public function hobby_add()
    {
        if ($_POST) {
            ...
            $this->load->view("mgr/users_form", $this->data);
        }
    }
