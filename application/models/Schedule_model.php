<?php defined('BASEPATH') or exit('No direct script access allowed');
    ...
    class Schedule_model extends Base_Model {
	    ...
	    {
		    $total = $this->db->select()->from('schedule')->where($syntax)->get()->num_rows();
        ...
        public function edit($home_id, $data){
            return $this->db->where(array("id"=>$home_id))->update('schedule', $data);
        }
        ...
        public function get_data($home_id){
            return $this->db->get_where('schedule', array("id"=>$home_id))->row_array();
        }


        public function get_all_post(){
            $str="select * from schedule where is_delete=0 ";
